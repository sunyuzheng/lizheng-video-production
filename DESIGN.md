...
结构合法不等于语义断句可读。真实失败中，低标点 ASR 被固定窗口和字数上限连续装箱，长度、时间与 CPS 全部合格，仍把完整意思切成了机械短句。因此 splitter 记录每个边界来自句／子句标点、真实停顿、固定窗口还是 token fallback；整片标点与 fallback 比例用于解释问题，真正的阻断以连续的近上限 fallback 区段为主，避免局部坏段被全片平均值稀释，也避免分散的正常 fallback 被平均值误杀。短材料不自动阻断，留给人工快速复核。

这个 gate 只识别一种高风险退化，不声称机器已经理解全部语义。重断句可以补标点、改 cue 和时间边界，但正文字符流必须与 corrected 源一致；错字与实体修正仍由 correction 阶段负责，不通过放宽其局部替换协议来兼做全片标点恢复。
...
开头不是独立的创意步骤，而是标题与封面承诺的第一次兑现。一般高光仍用于理解材料；成片开头在主包装确定后重新选择。原片能用少量片段完成“确认期待 → 增加张力 → 进入答案”时做 cold open；命题需要跨片段综合时由主持人补录 narrative intro。系统交付经字幕核对的 cue-level in/out、原话或补录逐字稿，不自动改写成片；说话人身份只在 sidecar 与当前 SRT 匹配时沿用，因此两种方案仍可作为编辑实验。
