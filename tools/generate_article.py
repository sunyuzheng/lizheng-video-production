...
先在心里写一句具体判断，并标明它来自谁、由哪个事实或现场支撑。好的文章不需要宣布自己重要；判断的分量来自它的来路。

### 先确定真实读者
从逐字稿、用户说明或已有产品资料理解读者已经有什么能力、处在什么阶段、正在被什么卡住。好的读者判断会让一个具体的人在处境里认出自己；它不是一串产品类型或身份标签。

### 保护嘉宾带来的成熟框架
如果访谈讨论的是一本书、一个已有方法或命名框架，先理解它的原名、顺序、定义、例子和内部逻辑。编辑的价值是让这套智识结构变得可见，而不是用更整齐、更熟悉的包装取代它。现场怎样试、哪里失败、怎样修正，和整理后的答案同样重要。
...
### 让 substance 先于 commentary
把篇幅留给事实、判断、原话和推理。编辑评价只在它能增加理解时出现。读者应该先遇到内容本身，再自然感到它重要；文章的力量来自所选择的材料及其关系，不来自作者替材料颁奖。
...
- 留意统一排比、连续硬转折、过度罗列和抽象套话。这些不是机械禁区；判断标准是它们有没有压平作者原本的语气和材料的复杂度。
...
- 开头优先给一个可核验的事实、数字、动作或现场，让读者先进入材料，再理解编辑的判断。
- 理解真实读者已经有什么能力、处在什么阶段、缺什么。让对方在具体处境里认出自己，而不是用产品类型圈出一个泛化人群。
- 嘉宾带来成熟框架时，先把它的智识结构讲透：原名、顺序、定义、例子以及各部分为什么这样连接。编辑的表达应增强原框架，而不是取代它。
- 选择一条最能承载这场对话的主线。其他话题按它们对主线的贡献获得不同篇幅，允许文章有明显的轻重和不对称。
- 让具体场景贴着判断出现：谁做了什么、花了多少、哪里卡住、怎样修正。原话是推理的一部分，不是总结之后补上的装饰。
- 引用可以轻微清理口语，但要保留说话人的思想纹理、犹豫和精确含义。
- 观看地图通常放在正文之后，用真实时间戳承接没有进入主线的内容；如果前置更符合阅读需要，也可以调整。
- 第一人称只承接逐字稿中真实发生的经历、选择和判断。主持人的现场失手有时比整理后的正确答案更有价值。
- 结构从素材的轻重长出来。小节数量、长度和节奏可以不整齐；文章不必看起来像一个标准答案。
...
输出前以三个视角重读，而不是按清单把文章磨平：
- **Substance**：文章有没有保住素材里真正独特的智识结构，事实与判断是否咬合。
- **Voice**：作者的思考方式和说话纹理是否还在，编辑有没有用统一腔调盖过它。
- **Reader**：那个具体读者能否认出自己的处境，并得到比泛化总结更精确的新理解。
