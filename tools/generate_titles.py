...
用法：
  ...
  python3 tools/generate_titles.py episode.final.srt         # 直接从完整字幕选题
...
原片逐字稿与文章同时存在时，以逐字稿找证据，文章作为辅助理解。先保住尚未解决的具体矛盾、动作和现场转折，避免把它们压成一个观众以为早懂的道理。观看回报也可以是向往、人物兴趣、直接解答或情感体验，不必都包装成损失焦虑或悬念。

对每个真正有潜力的交点，写清：

- 观众在点开前已经存在的观看动机；
- 观众可能以为自己已经知道的答案，本期仍值得看的增量，以及它如何在包装里显出来；
- 材料给出的独有证据、机制或答案，以及可核对的时间点或原文；
- 一句话 premise；
- 拿掉编辑解释后，标题与封面显出的观看期待；若靠好奇驱动，写出具体问题；
...
不要给整期写摘要。把 brief 中最强的几个“观众原有观看动机 × 材料独有证据”分别做成标题 × 封面组合；每组表达一份清楚的观看回报。标题可以打开情境、因果或矛盾，封面用一个人物、数字或视觉对比让它变得可见。两者可以适度重复；可以透露结论，只要过程、证据、反例或体验仍值得看。悬念本身是主要回报时，再保护相应揭晓。
...
- 完整视频标题；
- 封面主文案与一个可执行的视觉关系；
- 只看标题与封面时，观众脑中被打开的那一个问题，或想获得的明确答案／过程／体验；
...
找出少量最强交点：核心观众能认出值得花时间的回报，本期又有独有的事实、机制或经历支撑。先找可能被摘要磨平的具体矛盾，再判断观众是否会误以为“这不就是在说……”。一个强段落可以承担整期包装；篇幅、信息量或数字大小不决定优先级。

为每个交点提出一组标题 × 封面，并写明观众预期获得什么或会自然追问什么、材料在哪里兑现、开头如何接住，以及最可能的误读。用陌生信息流中的快速识别和稍后复述检验组合。人物或事件本身若已有足够意义，可以形成真实动机；否则不要强行映射到个人用途。历史样本只校准观众曾为何付出注意力，不模仿句式。
"""
...
先只看每组标题与缩小后的封面，再读编辑解释。判断观众能否认出值得看的回报；是否会预判为自己早懂的泛泛道理；相关性是否需要一段话事后补上；第二人称是否只是在语法上假装亲近。人物、事件、向往或体验本身足够吸引人时，不需要强行改写成个人用途。

同时检查组合是否表达同一份清楚承诺、十分钟后能否复述、视频能否具体兑现、开头能否迅速证明观众没有点错。先比较不同观看承诺，再比较同一角度的措辞；给终审明确的存废和排序。模型冷读和排序不是观众实测，不声称已证明点击率提升。若两轮都错过了更强交点，可以提出新组合。{highlight_alignment_check}
"""
...
开头先确认观众从标题与封面带进来的期待，通过新的事实、矛盾、故事或直接开始解答让观看继续值得。可以先亮判断或结果，只要后续过程、证据或体验仍有价值。不要另造无关 hook，也不要在已经声明“这期会讲什么”之后继续堆履历、背景或 roadmap。对访谈，根据完整材料选择更可靠的一条路：
...
若正文缺少关键答案、案例或推演，首选改用现有材料能兑现的承诺；在放弃方向中说明原角度需要补什么内容，必要时建议重录。不要用补录 hook 冒充缺失的正文，也不执行本次未要求的剪辑或重录任务。

## 输出文件格式（这是给剪辑师和主播单独阅读的交付文件，不是给你自己的笔记）

固定三段结构，顺序不可变：

1. `## 首选组合`——置顶，只放一个选择。依次使用独立字段行 `**标题：**`、`**封面主文案：**`、`**封面画面：**`、`**观众会追问：**`、`**视频兑现：**`、`**开头衔接：**`。`观众会追问` 写包装真正带来的问题或观看期待；可以是明确解答、过程或体验，不强造谜题，也不写受众说明。兑现尽量给时间点或原文线索。`开头衔接` 下必须另起字段：
...
def generate_titles(
    ...
    # 文章可能已经把原片的矛盾提炼成道理；完整字幕同时服务选题与开头。
    if content_path.suffix.lower() == ".md" and resolved_source_srt is not None:
        content = (
            "## 完整原片逐字稿（选题证据）\n\n"
            f"{srt_to_text(resolved_speaker_srt or resolved_source_srt)}\n\n"
            "## 辅助文章（不能替代原片证据）\n\n"
            f"{content}"
        )
