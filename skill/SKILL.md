...
description: 给定视频、音频或已有字幕，完成本地转写、字幕精校、断句，并按视频类型（单口/访谈）分流产出：高光选取、文章（单口外发稿或访谈伴读稿）、标题（频道多轮标题，或经 xhs-cover-title 产出小红书封面+标题）、访谈封面图。转写用 mlx-qwen3-asr 1.7B，精校用 Codex CLI，内容生成用 Claude Code Fable 5，不可用时降级 Codex gpt-5.5。
---

# KDB 视频后期生产 v2

一段原始录制 → 四类资产：**①精校字幕 ②高光 ③文章 ④标题**（+ 访谈封面图一个可选件）。
...
用 **Codex CLI 侧的 `imagegen` skill**（不是 Claude skill，产物落在 `$CODEX_HOME/generated_images/`）。优先用用户提供的现场照、视频截图或关键帧作 reference，生成 16:9 播客访谈风格封面：双人对谈清晰、光影构图到封面级、保留可加标题的留白。**不生成文字、logo 或水印**（除非用户给了准确文案并明确要入图）——路线 B 产出的封面文案就是为这块留白准备的字，加字属于剪辑/设计工序。最终图必须拷贝到视频同目录存为 `<video>.cover.png`，不能只留在 generated_images。
...
| `<video>.cover.png` | 访谈发布默认 | imagegen（Codex 侧） |
