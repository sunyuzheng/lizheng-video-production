...
description: 为课代表立正视频完成字幕精校、断句、VTT/QC、高光、description 与多资产后期编排；单独封标用 video-title-and-cover，已有访谈材料只写文章用 expert-interview-article。
---
...
| 请求 | 主责 |
|---|---|
| KDB 视频／SRT → 字幕、高光、description 或多资产组合交付 | 本 skill；封标阶段交给 `video-title-and-cover` |
...
编辑判断、成对标题文案、封面制作与包装对应的开头由独立的 [`video-title-and-cover`](../skills/video-title-and-cover/SKILL.md) 主责，可单独安装和使用。组合视频任务在此处加载它；只改一个词或快速 brainstorm 时，不要求运行多轮脚本。原有 Python 标题入口仍可用：
...
脚本读取的 `data/guideline_kedaibiao.md` 是新 skill 编辑正文的仓内相对链接；标题和高光都使用同一维护源。它保留完整源材料、brief、独立 challenger 与终审的现有契约，多轮模型排序仍只是编辑辅助。

主流水线自动把 final SRT 传作开头定位材料，并在存在时采用与之匹配的 speaker sidecar；单独以文章运行也应传入 `--source-srt`，必要时可加 `--speaker-srt`。没有带时间逐字稿时可以设计补录 intro，不能声称原片 cold open 已经可执行；没有通过校验的 sidecar 时不强行归为主持人或嘉宾。
...
- **封面**：由 `video-title-and-cover` 制作和验收实际图片；比例与交付范围服从当前请求，完整双平台任务使用独立 16:9／3:4。旧 `references/cover-style-guide.md` 链接到该 skill 的制作规范；品牌资产按需路由 `superlinear-brand-usage`。
...
4. 封面是否先筛选自然表情，再对照原片检查两个人的相貌、眼神、嘴型和肤色（见 `references/cover-style-guide.md`）；按本次偏好允许适度提亮、修饰与轻微重绘；明确要求原图时保留原图人物，以自然、像本人、精神为准。只改文字的版本也快速复核人物，缩略图下检查文字与品牌。
5. 用户只要求草稿时是否始终停在草稿；发布前是否展示最终 payload、目的地与受众并取得批准。

## 持续校准

把反馈更新到最准确的 owner：字幕机制进脚本与测试，封标判断与真实返工进 `video-title-and-cover`，频道历史样本仍进 `data/top_titles.txt`，文章判断进对应 writing skill，品牌规则进 `superlinear-brand-usage`。记录原理、适用条件和失败模式，不把一次修改追加成永久禁句表。
