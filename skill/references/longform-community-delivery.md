...
1. 帖子标题；
2. Circle 原生视频（关联 VTT 和 thumbnail）；
3. 经过 Substance、Voice、Reader 三种视角重读的伴读文章正文；
4. 带真实时间戳的观看章节；
5. 可选的讨论问题；只有当问题能把读者带回自身经验或选择，而不是泛泛询问「最大的启发」时，才值得加入。

帖子包装服务文章主线。通常不需要在文章前后再堆一套泛化的「适合谁、为什么值得看、你将收获什么」；如果这些信息确实增加理解，可以自然融入，而不是另起一份模板。
